...
#' \code{display} and \code{bland.altamn.plot} functions. However
...
#' statistics( measurement1 , measurement2 )
#'
#' # Generates Bland-Altman statistics data of the two measurements using the formula interface
#'
#' statistics( measurement2 ~ measurement1 )
#'
#' # Example with a real data set
#' statistics( Method.B ~ Method.A, data = giavarina.2015 )
...
statistics.default <- function(x,
    ...
    method1 = x
    method2 = y
    ba.data <- data.preparation( method1 , method2 , sig.level )
...
#' @rdname statistics
#' @export
statistics.formula = function(formula, data = parent.frame(), ...){

  # alows a formula like: results = statistics(Method.B ~ Method.A, data = giavarina.2015)
